...
'''
sv = hoshino.Service('涩图', help_=HELP_MSG)
...
def render_forward_msg(msg_list: list, ev, name='ネギ'):
	forward_msg = []
	for msg in msg_list:
		forward_msg.append({
			"type": "node",
...
async def send_msg(msg_list, ev):
	...
	if not get_config('base', 'enable_forward_msg'):
		...
		forward_msg = render_forward_msg(msg_list,ev)
		try:
...
async def send_setu(bot, ev):
	...
	if not is_su:
		...
		if args[2] == 'true' or args[2] == 'on':
			...
			value = int(args[2])
		elif args[1] == 'night' or args[1] == 'day':
			value = args[2]	
		else:
			value = None
